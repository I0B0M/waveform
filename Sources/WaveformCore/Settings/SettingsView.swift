        ...
        Form {
            Section {
                ...
                if hotkeyPreset == .fnTap {
                    Text("Tap fn (🌐) to start, tap again to stop — or hold it down for 2+ seconds to push-to-talk: release inserts immediately. Using fn as a modifier (fn+arrows, fn+delete) never triggers it. If macOS also opens the emoji picker on fn, set System Settings → Keyboard → “Press 🌐 key to” → Do Nothing. Uses the same Accessibility permission as inserting text — no extra grants.")
                        .font(.caption)
